...
knitr::opts_chunk$set(echo = TRUE, fig.width=20, fig.height=8)
library(reefbudgetR)
library(tidyverse)
library(dplyr)
options(digits = 3)
...
net_site$CB_METHOD <- factor(net_site$CB_METHOD_BENTHIC, levels = c("CPM", "Chords", "SfM"))
...
```{r plot_parrotfish}
parrotfish_plot <- ggplot(data = subset(net_site, CB_METHOD_BENTHIC = "CPM")) +
  geom_col(
    aes(x = OCC_SITENAME,
...
net_plot <- ggplot(data = net_site) +
  ...
  scale_fill_manual(values = c("#0085ca", "#54addb", "#a6d4ec"),
                    name = "") +
  #scale_x_discrete(labels = cpm$Label) +
  scale_y_continuous(
    limits = c(-2, 16),
